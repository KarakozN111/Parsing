...
# отдельные категории для lg, samsung
category_pages = {
...
try:
    ...
    print("\nОстановка пользователем! Сохраняю уже собранные данные...")
finally:
    driver.quit()
    ...
    if products_data:
        ...
        df['Price'] = df["Price"].apply(
            lambda x: int(''.join(re.findall(r'\d+', x))) if isinstance(x, str) and re.findall(r'\d+', x) else None
        )
        ...
        print(f"\n Парсинг завершён. Сохранено строк: {len(df)}")
    else:
